...
// RR = 3
// new a static to keep record on how many times scheduler been called
// void schedule(void){
//     if( is_thread_start == 0 )
//     {
//         // execute the first thread in wait_queue at time==0
...
//     else
//     {   
//         // printf("ID: %d, cnt: %d, exit: %d, yield: %d, time: %d\n", current_thread->ID, schedule_cnt, current_thread->is_exited, current_thread->is_yield, current_thread->remain_execution_time);
//         schedule_cnt += 1;
//         if (current_thread->is_exited == 1 || current_thread->is_yield == 1 || schedule_cnt == 3) {
//             schedule_cnt = 0;
//             current_thread = current_thread->next;
...
//                 } else if (tmp_thread->remain_execution_time == shortest_thread->remain_execution_time) {
//                     if (tmp_thread->ID < shortest_thread->ID)
//                         shortest_thread = tmp_thread;
//                 }
...
//         schedule_time = uptime();
//         // printf("ID: %d, cnt: %d, exit: %d, yield: %d, time: %d\n", current_thread->ID, schedule_time, current_thread->is_exited, current_thread->is_yield, current_thread->remain_execution_time);
//         if ( current_thread->is_exited == 1 || current_thread->is_yield == 1 || (schedule_time-current_time)%3 == 0) {
//             tmp_thread = current_thread->next;
//             shortest_thread = current_thread;
//             if (current_thread->is_exited == 1) {
//                 shortest_thread = current_thread->next;
...
void thread_start_threading(int time_slot_size){
    // every __time_slot_size will call my_thrdstop_handler()
