...
sys_open(void)
{
  ...
  if(!(omode & O_NOFOLLOW)){
    int counter = 0;
    while(1){
      if((ip = namei(path)) == 0) // file does not exist in the path
        break;
      ilock(ip);
      if(ip->type != T_SYMLINK){  // file is not a symlink and stop following.
        iunlockput(ip);
        break;
      }
      if(readi(ip, 0, (uint64)path, 0, sizeof(path)) != sizeof(path)) // a->b->c.txt
